...
class MachineWindow(QFrame):
    ...
    def __init__(self, table_name):
        ...
        #print("params_L на подпись", self.params_L)
        #print("params для поиска", self.params)
        #print("PARAM_LIMдля словаря", self.PARAM_LIM)
    ...
    def update_graph(self):
        #print("current_param_index", self.current_param_index)
        param = self.params[self.current_param_index]#для вычислений
        #print("param", param)
        ...
        self.figure.clear()
        ax = self.figure.add_subplot(111)
        #print("Данные для графика values: ", values)
        ax.plot(values, color='#4CAF50')#загрузка данных в график
        ax.set_ylim(0, 300)
        ax.set_facecolor('#FAFAFA')
        ax.tick_params(axis='both', labelsize=7)
        for label in ax.get_yticklabels():
        ...
        #print("len(param_L)", len(param_L))
        #print("self.current_param_index", self.current_param_index)
        #print(" ")
